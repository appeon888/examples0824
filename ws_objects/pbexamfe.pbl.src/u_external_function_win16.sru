...
public function unsignedlong uf_get_logon_time ();//user gettickcount to find total logon time
Return GetTickCount()
...
protected function string uf_get_pb_ini ();String	ls_PBINI

// Get the location from the WIN.INI
ls_PBINI = ProfileString("win.ini", "powerbuilder", "InitPath050", "x")
If ls_PBINI = "x" Then
	// InitPath050 does not exist, look for InitPath
	ls_PBINI = ProfileString("win.ini", "powerbuilder", "InitPath", "x")
End If

If ls_PBINI = "x" Then
	ls_PBINI = "pb.ini"
Else
	ls_PBINI = ls_PBINI + "\pb.ini"
End If
